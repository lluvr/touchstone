...
| `mcp_programmatic.py` | Programmatic invocation of the Touchstone MCP server's four tools (`verify`, `measure`, `assess_derivation_regime`, `list_modes`). Requires the `mcp` extra (`pip install -e ".[mcp]"`). Useful for verifying the server works locally and as a starting point for embedding it in a custom transport. | Inline fixtures |
