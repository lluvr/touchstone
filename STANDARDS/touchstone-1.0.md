...
**Status:** Draft v0.7. All Section 4-12 content substantively complete, including Terminology (§2), Output structure (§4), Conformance declaration mechanism + invalidation criteria (§11), confirmed reference-test conformance bands (§8) disambiguated against the regression reference, structured Field positioning references (§12), falsifiable construct claims (§3.5) updated with two external corpus results (RAGTruth Summary and SummEval), and Appendix C citing public-surface validation artifacts only. Section 6 (Specification Compliance) and Appendices A and B remain reserved for Standard 1.1. Independent editor review is pending.
...
- **Layer 10 (quality_profile gap).** Falsified if, on a corpus where the faithful/embellished distinction has been independently re-annotated by a non-Clarethium party (e.g., a TRUE / LLM-AggreFact / HaluBench / HaluEval / RAGTruth / SummEval subset), the gap signal fails to discriminate the two conditions at Cohen's d magnitude > 1.0 (a large effect). Status as of `1.0.0-draft.7`: **partially falsified out-of-domain across two corpora.** On the RAGTruth Summary test split (n=900, MIT-licensed, multi-vendor; the `benchmarks/external/ragtruth_summary/` run), Layer 10 gap shows AUC-ROC 0.4981 against the per-output hallucination label, equivalent to d ≈ 0; substance components fire on only 3% of outputs (`source_fidelity` 0.7%, `entity_grounding` 2.6%, `epistemic_calibration` 0.1%). On the SummEval test corpus (n=1600 (article, summary) pairs, CNN/DM, the `benchmarks/external/summeval/` run), Layer 10 gap shows AUC 0.5000 against a binarized consistency label and Spearman ρ = 0.0000 against the continuous 1-5 consistency rating; substance components fire on 0% of outputs. The Layer 10 construct holds within its calibrated domain (long-form analytical Markdown with sufficient claim density) but does NOT generalize to short summary outputs in either corpus tested: the composite reduces to presentation-only when its substance-side preconditions are not met, and carries no fidelity information. The Standard's scope statement (§9.2) is updated accordingly: Layer 10 gap's substrate-generalization claim is conditional on input regime, not unconditional.
...
- **The "scoring substrate is independent" claim (§3.1).** Falsified if any non-Layer-1a layer's output is shown to depend on the choice of model that generated the input text in a way the layer's documented construct does not predict. The reference implementation runs identically on text from any generator; a measurement that shifts systematically with generator identity (beyond what Layer 1b/1c/10 are designed to surface) indicates an undocumented dependence. Status as of `1.0.0-draft.7`: on RAGTruth Summary, Touchstone's per-model AUC for Layer 6 (the strongest surviving signal out-of-domain) ranges 0.59-0.73 across six instruction-tuned LLM families (gpt-3.5, gpt-4, llama-2-7B/13B/70B, mistral-7B). On SummEval, the same Layer 6 signal achieves AUC 0.7530 (Spearman ρ -0.3481) across 16 older summarization-system architectures; the per-system breakdown is in the benchmark output JSON. The variation across both corpora is within the noise expected from per-system label-balance differences; no systematic model-identity dependence is observed beyond what label-balance variation would predict.
