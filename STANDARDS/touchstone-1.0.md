...
- **Layer 6 (vocabulary proximity).** Falsified if, on a corpus where each (output, source) pair has been manually rated for "lexical closeness to source" on a Likert scale (1-5), the layer's mean_proximity fails to correlate with the manual rating at Spearman ρ ≥ 0.4. The construct claim is that content-word overlap proxies lexical closeness; falsification would indicate the stopword-filtered, 3-character-minimum overlap metric no longer matches the construct.
