...
**Status:** Draft v0.15. All Section 4-13 content substantively complete. §13 (calibrated Verifier methodology) anchored by operational metrics on three external corpora and a 16-category subtle-hallucination stress test that documents where the substrate is structurally blind (direction reversal, attribute swap, scoping shift, relation reversal, time-frame shift, imputed cause). §13.6 explicitly names the two-stage production architecture (substrate + semantic discriminator) as the recommended deployment shape. draft.15 closes the doc-vs-library gap that draft.14's stress-test re-review surfaced: the `Verifier` class now ships a `substrate_plus_judge` mode and a `judge_hallucinated_prob` parameter on `.score()`, so the §4.2.8 multi-vendor / §4.3 substrate-plus-judge / §4.3.1 holdout-blend architecture the doc recommends is now one-line-deployable. Three places in the doc set that cited a non-existent `Verifier(use_minicheck=True)` API (README §"Read before deploying", §5 production architecture, §4.2.3 calibration implications) are corrected. The four pre-existing modes (`substrate_only`, `substrate_plus_minicheck`, `substrate_plus_minicheck_alignscore`) ship unchanged; the new mode is additive. Reference test suite at `tests/reference/cases/` ships 16 cases. Section 6 (Specification Compliance) and Appendices A and B remain reserved for Standard 1.1. Independent editor review is pending.
