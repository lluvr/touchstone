...
**Status:** Draft v0.8. All Section 4-12 content substantively complete, including Terminology (§2), Output structure (§4), Conformance declaration mechanism + invalidation criteria (§11), confirmed reference-test conformance bands (§8) disambiguated against the regression reference, structured Field positioning references (§12), falsifiable construct claims (§3.5) updated with three external corpus results (RAGTruth Summary, SummEval, HaluEval summarization), and Appendix C citing public-surface validation artifacts only. Section 6 (Specification Compliance) and Appendices A and B remain reserved for Standard 1.1. Independent editor review is pending.
...
- **Layer 10 (quality_profile gap).** Falsified if, on a corpus where the faithful/embellished distinction has been independently re-annotated by a non-Clarethium party (e.g., a TRUE / LLM-AggreFact / HaluBench / HaluEval / RAGTruth / SummEval subset), the gap signal fails to discriminate the two conditions at Cohen's d magnitude > 1.0 (a large effect). Status as of `1.0.0-draft.8`: **partially falsified out-of-domain across three independent corpora.** On the RAGTruth Summary test split (n=900, MIT-licensed, multi-vendor; the `benchmarks/external/ragtruth_summary/` run), Layer 10 gap shows AUC-ROC 0.4981, equivalent to d ≈ 0; substance components fire on 3% of outputs. On the SummEval test corpus (n=1600 (article, summary) pairs, CNN/DM, the `benchmarks/external/summeval/` run), Layer 10 gap shows AUC 0.5000 and Spearman ρ = 0.0000 against the continuous consistency rating; substance components fire on 0% of outputs. On the HaluEval summarization subset (n=1000 (article, summary) pairs across 500 documents with adversarially-synthesized hallucinations; the `benchmarks/external/halueval_summarization/` run), Layer 10 gap shows AUC 0.5020 and paired-ranking accuracy 50.2% with 490/500 document pairs producing identical scores on the right and hallucinated summary; substance components again fire on a negligible fraction of outputs. The Layer 10 construct holds within its calibrated domain (long-form analytical Markdown with sufficient claim density) but does NOT generalize to short summary outputs in any of the three corpora tested. The composite reduces to presentation-only when its substance-side preconditions are not met, and carries no fidelity information. The Standard's scope statement (§9.2) is updated accordingly: Layer 10 gap's substrate-generalization claim is conditional on input regime, not unconditional.
...
- **The "scoring substrate is independent" claim (§3.1).** Falsified if any non-Layer-1a layer's output is shown to depend on the choice of model that generated the input text in a way the layer's documented construct does not predict. The reference implementation runs identically on text from any generator; a measurement that shifts systematically with generator identity (beyond what Layer 1b/1c/10 are designed to surface) indicates an undocumented dependence. Status as of `1.0.0-draft.8`: Layer 6 inverse vocabulary proximity (the strongest surviving signal across the three external corpora tested so far) achieves AUC 0.6723 on RAGTruth Summary across six instruction-tuned LLM families, AUC 0.7530 on SummEval across 16 older summarization-system architectures, and AUC 0.7593 on HaluEval summarization (paired-ranking accuracy 80.3%, ChatGPT-synthesized adversarial hallucinations). The signal is remarkably stable across the three corpora (range 0.67-0.76) despite very different model populations and construction processes. The HaluEval result is partly explained by Layer 6 directly measuring what HaluEval's adversarial construction produces (vocabulary outside the article distribution); this is a construct-alignment artifact rather than independent evidence of generalization, and is documented in the benchmark README. No systematic model-identity dependence is observed beyond what label-balance and corpus-construction variation would predict.
