...
**Status:** Draft v0.13. All Section 4-13 content substantively complete. §13 (calibrated Verifier methodology) anchored by operational metrics on three external corpora and a 16-category subtle-hallucination stress test that documents where the substrate is structurally blind (direction reversal, attribute swap, scoping shift, relation reversal, time-frame shift, imputed cause). §13.6 explicitly names the two-stage production architecture (substrate + semantic discriminator) as the recommended deployment shape. The Standard now positions Touchstone as the lexical-feature half of a production hallucination detector, not as a standalone audit-grade detector. The canonical reference suite at `tests/reference/cases/` ships 16 cases. Section 6 (Specification Compliance) and Appendices A and B remain reserved for Standard 1.1. Independent editor review is pending.
