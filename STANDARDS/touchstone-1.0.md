...
**Status:** Draft v0.9. All Section 4-12 content substantively complete, including Terminology (§2), Output structure (§4), Conformance declaration mechanism + invalidation criteria (§11), confirmed reference-test conformance bands (§8) disambiguated against the regression reference, structured Field positioning references (§12), falsifiable construct claims (§3.5) updated with three external corpus results × three task types and 95% bootstrap CIs on all reported AUCs, and Appendix C citing public-surface validation artifacts only. Section 6 (Specification Compliance) and Appendices A and B remain reserved for Standard 1.1. Independent editor review is pending.
...
- **Layer 10 (quality_profile gap).** Falsified if, on a corpus where the faithful/embellished distinction has been independently re-annotated by a non-Clarethium party (e.g., a TRUE / LLM-AggreFact / HaluBench / HaluEval / RAGTruth / SummEval subset), the gap signal fails to discriminate the two conditions at Cohen's d magnitude > 1.0 (a large effect). Status as of `1.0.0-draft.9`: **partially falsified out-of-domain across three independent corpora and three task types.** Touchstone Layer 10 gap AUC-ROC point and 95% percentile bootstrap CIs (1000 stratified resamples, fixed seed) across six (corpus, task) cells: RAGTruth Summary 0.4981 [0.4830, 0.5111], RAGTruth QA 0.5127 [0.4985, 0.5295], RAGTruth Data2Txt 0.5041 [0.4908, 0.5170], SummEval 0.5000 [0.5000, 0.5000], HaluEval summarization 0.5020 [0.4950, 0.5090]. All five 95% CIs include 0.5000; the Layer 10 gap signal is statistically indistinguishable from chance on every out-of-domain corpus and task type tested. Substance components fire on a negligible fraction of outputs across all six cells (3% maximum on RAGTruth Summary, 0% on SummEval, near-zero on HaluEval), so the composite reduces to presentation-only and carries no fidelity information. The Layer 10 construct holds within its calibrated domain (long-form analytical Markdown with sufficient claim density, EXP-081 internal corpus d = -5.238) but does NOT generalize to short summary outputs in any of the corpora tested. The Standard's scope statement (§9.2) is updated accordingly: Layer 10 gap's substrate-generalization claim is conditional on input regime, not unconditional.
