...
**Status:** Draft v0.14. All Section 4-13 content substantively complete. §13 (calibrated Verifier methodology) anchored by operational metrics on three external corpora and a 16-category subtle-hallucination stress test that documents where the substrate is structurally blind (direction reversal, attribute swap, scoping shift, relation reversal, time-frame shift, imputed cause). §13.6 explicitly names the two-stage production architecture (substrate + semantic discriminator) as the recommended deployment shape. draft.14 ships four critical-tier honesty fixes to the §4 cross-detector evidence in `docs/production_readiness.md`: (1) the xAI Grok judge prompt is now named a `cued` variant with a paired `blind` variant exposed via `--prompt-variant`, and the §4.1 confounds box adds prompt-cueing as a load-bearing methodological concern with the in-session three-arm stress-test result included; (2) the §4.2 sampling vocabulary is corrected from "stratified" to "deterministic first-N prefix" everywhere with the base-rate-preservation defense made explicit; (3) §4.2.1 adds held-out F1-optimal threshold selection (200/200 stratified interleave) showing in-sample-vs-holdout F1 inflation up to 0.234 on substrate L6 / SummEval; (4) §4.2.2 adds the K=100 tie-aware metric envelope showing Grok R@P90 tie-sensitivity (SummEval "12 of 46" is 7 ± 3 under random tie-break; HaluEval "87 of 200" is 67 ± 16). The canonical reference suite at `tests/reference/cases/` ships 16 cases. Section 6 (Specification Compliance) and Appendices A and B remain reserved for Standard 1.1. Independent editor review is pending.
**Version:** 1.0.0-draft.14
**Date:** 2026-05-18 (drafting in progress)
**License:** CC-BY 4.0
...
Each layer's construct claim is falsifiable in principle. The Standard names the evidence that would invalidate each. Falsifiable claims for the required layers (4, 10, 11, plus the §3.1 substrate-independence claim) carry status notes recording the current empirical evidence; falsifiable claims for the remaining required layers (1b, 1c, 2, 3, 5, 6, 7) and the experimental layers (8, 9) name the falsification evidence but do not yet record status against external corpora.

- **Layer 1a (heading defaultness).** OPTIONAL; LLM-dependent. Falsified if, on a corpus where document headings are hand-classified into "default for the topic" vs "non-default", the layer's `is_default` flag (computed from word-level Jaccard overlap against caller-supplied baseline headings) disagrees with the manual classification on more than 30% of documents at the default 0.4 overlap threshold. Layer 1a depends on the caller's LLM client and its prompting parameters; falsification reports MUST document the baseline-generation configuration.
- **Layer 1b (mechanism ratio).** Falsified if, on a corpus where each sentence has been hand-classified into "causal-reasoning" vs "buzzword-laden", the layer's mechanism_ratio fails to rank the documents in the same order as the human classifiers at Spearman ρ ≥ 0.5. The construct claim is that the regex patterns proxy reasoning-style density; falsification would indicate the patterns no longer match the construct.
- **Layer 1c (assertion ratio).** Falsified if, on a corpus where each sentence has been hand-classified into the five register categories (ASSERTION, QUALIFIED, CONDITIONAL, EVIDENCED, SPECULATIVE), the layer's category counts disagree with manual counts by more than 20% in aggregate when the sentence count is ≥ 50. The five-category regex set is the operational definition; falsification indicates the patterns no longer cover the categories they name.
- **Layer 2 (claim density).** Falsified if, on a corpus where every digit-formatted claim has been manually annotated, the layer's extraction recall drops below 90% or its false-positive rate exceeds 5%. The construct claim is exhaustive coverage of digit-formatted numerical claims as defined by the six pattern types (percentage, dollar, multiplier, entity count, duration, range). Falsification triggers a pattern-set update.
- **Layer 3 (temporal instability).** Falsified if, on a corpus where multi-version outputs have been hand-classified for unstable claims (claims present in some versions but not all), the layer's instability_rate disagrees with the manual count by more than 10 percentage points in aggregate. The construct claim is that instability is an upper bound on fabrication (a claim that varies across re-generations cannot be a faithful copy of source); falsification would not retire the layer (the upper-bound framing is intact) but would indicate the extraction or matching logic has drifted.
...
- **Layer 10 (quality_profile gap).** Falsified if, on a corpus where the faithful/embellished distinction has been independently re-annotated by a non-Clarethium party (e.g., a TRUE / LLM-AggreFact / HaluBench / HaluEval / RAGTruth / SummEval subset), the gap signal fails to discriminate the two conditions at Cohen's d magnitude > 1.0 (a large effect). Status as of `1.0.0-draft.13`: **partially falsified out-of-domain across three independent corpora and three task types.** Touchstone Layer 10 gap AUC-ROC point and 95% percentile bootstrap CIs (1000 stratified resamples, fixed seed) across five (corpus, task) cells: RAGTruth Summary 0.4981 [0.4830, 0.5111], RAGTruth QA 0.5127 [0.4985, 0.5295], RAGTruth Data2Txt 0.5041 [0.4908, 0.5170], SummEval 0.5000 [0.5000, 0.5000], HaluEval summarization 0.5020 [0.4950, 0.5090]. All five 95% CIs include 0.5000; the Layer 10 gap signal is statistically indistinguishable from chance on every out-of-domain corpus and task type tested. Substance components fire on a negligible fraction of outputs across all five cells (3% maximum on RAGTruth Summary, 0% on SummEval, near-zero on HaluEval), so the composite reduces to presentation-only and carries no fidelity information. The Layer 10 construct holds within its calibrated domain (long-form analytical Markdown with sufficient claim density, EXP-081 internal corpus d = -5.238) but does NOT generalize to short summary outputs in any of the corpora tested. The Standard's scope statement (§9.2) is updated accordingly: Layer 10 gap's substrate-generalization claim is conditional on input regime, not unconditional.
- **Layer 5 (entity provenance).** Falsified if, on a corpus where named entities have been manually extracted and verified against the source, the layer's entity_unsourced_rate disagrees with the manual rate by more than 15 percentage points in aggregate. The construct claim is that the regex-based extraction (person, organization, attribution, parenthetical citation, CamelCase) covers the entity types named in §5.5; lowercase substring matching against source is the operational sourced-vs-unsourced rule.
- **Layer 6 (vocabulary proximity).** Falsified at the construct level if, on a corpus where each (output, source) pair has been manually rated for "lexical closeness to source" on a Likert scale (1-5), the layer's mean_proximity fails to correlate with the manual rating at Spearman ρ ≥ 0.4. The construct claim is that content-word overlap proxies lexical closeness; failure to correlate would indicate the stopword-filtered, 3-character-minimum, per-sentence overlap metric no longer matches the construct. Status as of `1.0.0-draft.13`: Layer 6 inverse_proximity is statistically indistinguishable from a 3-line raw WordOverlapInv baseline across three external corpora (RAGTruth Summary, SummEval, HaluEval summarization; CIs heavily overlap on every corpus per `benchmarks/external/*/results/trivial_lexical_baselines_2026-05-17.json`). The Layer 6 construct claim is therefore **not the discriminative claim** that the layer's AUC adds value beyond trivial lexical overlap; it is the calibration claim that the stopword-filtered, per-sentence formulation produces a more stable signal across preprocessing variants than any single trivial baseline (cross-corpus SD: Layer 6 0.039 vs JaccardContentInv 0.106, TFIDFCosineInv 0.065; WordOverlapInv 0.026 is tighter still, so plain word overlap is the genuinely substrate-independent baseline at this signal-strength tier). The honest framing in the README §Empirical validation reflects this finding.
- **Layer 7 (presentation features).** Per §7.5, Layer 7 is descriptive (no normative pass/fail thresholds). Falsified if any individual feature calculation (type-token ratio, Flesch-Kincaid grade level, formatting density, assertiveness ratio, named concept count) disagrees with an independent re-computation of the same formula on the same input by more than 2% relative error. This is a correctness-of-calculation criterion, not a construct-validity criterion.
- **Layer 8 (epistemic calibration).** EXPERIMENTAL in v1.0. Falsified if, on a corpus where each assertion-bearing sentence has been hand-classified as "grounded in source" vs "overclaiming", the layer's calibration_score disagrees with the manual rate by more than 20 percentage points in aggregate when the layer's precision flag is at least "adequate" (≥5 assertion-bearing sentences). The construct claim is that the three grounding signals (sourced number, sourced entity, vocabulary overlap > 50%) together approximate semantic grounding.
- **Layer 9 (information novelty).** EXPERIMENTAL in v1.0; length-confounded per §5.9. Falsified if, on a corpus where each sentence has been hand-classified as "introduces new information" vs "repeats prior content", the layer's per-sentence novelty score disagrees with the manual labels at AUC ≥ 0.6 (since the construct is acknowledged to be length-confounded, the falsification threshold is lower than for non-experimental layers). The construct claim is restricted to lexical novelty; semantic novelty is explicitly out of scope.
...
- **The "scoring substrate is independent" claim (§3.1).** Falsified if any non-Layer-1a layer's output is shown to depend on the choice of model that generated the input text in a way the layer's documented construct does not predict. The reference implementation runs identically on text from any generator; a measurement that shifts systematically with generator identity (beyond what Layer 1b/1c/10 are designed to surface) indicates an undocumented dependence. Status as of `1.0.0-draft.13`: Layer 6 inverse vocabulary proximity (the strongest surviving Touchstone signal across the external corpora tested) achieves AUC and 95% bootstrap CI as follows: RAGTruth Summary 0.6723 [0.6296, 0.7116], RAGTruth QA 0.6984 [0.6579, 0.7361], RAGTruth Data2Txt 0.6397 [0.6001, 0.6757], SummEval 0.7530 [0.7145, 0.7951], HaluEval summarization 0.7593 [0.7285, 0.7879]. Five (corpus, task) cells, all CIs disjoint from the 0.5000 chance level; Layer 6 AUC band 0.64-0.76 across the matrix. Layer 4 unsourced_rate spikes to AUC 0.7603 [0.6907, 0.8260] on RAGTruth QA where number-density gates it in (n=277/900); on summary outputs the signal is gated out for most examples. Two independently-trained LLM-based baselines (MiniCheck Flan-T5-Large, AlignScore-base) span a much wider AUC band than Touchstone L6 across the same five cells: MiniCheck Flan-T5-Large 0.7125 [0.6683, 0.7573] / 0.6437 [0.5978, 0.6920] / **0.4871 [0.4494, 0.5283]** (chance) / 0.8978 [0.8661, 0.9275]* / 0.6752 [0.6436, 0.7069]; AlignScore-base 0.7368 [0.7006, 0.7699] / 0.8091 [0.7714, 0.8455] / 0.6879 [0.6567, 0.7187] on the three summarization corpora. *Training-test leakage on SummEval (MiniCheck only). On RAGTruth Summary, both baselines and Touchstone L6 are statistically similar (overlapping CIs). On RAGTruth QA, Touchstone Layer 4 unsourced_rate beats MiniCheck (CIs disjoint by a small margin); Layer 6 also outperforms MiniCheck. On RAGTruth Data2Txt, **MiniCheck is statistically indistinguishable from chance** (CI includes 0.5000); Touchstone L6 (0.6397 [0.6001, 0.6757]) is statistically above chance and above MiniCheck (CIs disjoint). On HaluEval summarization, both LLM-based baselines underperform Touchstone L6 (the HaluEval result is partly explained by Layer 6 directly measuring what HaluEval's adversarial construction produces — vocabulary outside the article distribution — and is documented in the benchmark README). The cross-task variability of MiniCheck (0.49-0.90 across five cells; SD 0.16) versus the stability of Touchstone L6 (0.64-0.76 across the same cells; SD 0.05) is the strongest single piece of evidence for substrate independence: Touchstone's regex/arithmetic substrate produces a more uniform signal across input regimes than a single-model fine-tuned discriminator. AlignScore's intermediate stability (corpus-dependent CIs on the three summarization corpora; QA/Data2Txt baselines not computed in this round) is consistent with the general pattern. No systematic model-identity dependence is observed beyond what label-balance, task-type, and corpus-construction variation would predict.
...
**Reference suite.** The minimal canonical conformance subset is at `tests/reference/cases/`. Each case is a language-agnostic JSON document specifying inputs and per-layer expected outputs with a per-case absolute tolerance; the runner at `tests/reference/test_reference_cases.py` is pytest-discoverable so the suite runs as part of the default test matrix. Initial v1.0 coverage (16 cases) spans all required layers (1b, 1c, 2, 3, 4, 5, 6, 7), both experimental layers (8, 9), and Layer 11, including multi-currency extraction, gated precision-low behaviour, and a projected-content case for Layer 11. Additional edge cases (scaled integers, saturated derivation regime, multi-version temporal pairs) land in future revisions per `SUGGESTIONS/PROCESS.md`.
...
1. **Passes the conformance surface**, defined as: every case under `tests/reference/cases/` (the canonical reference suite; language-agnostic JSON cases verifying byte-pinned canonical layer outputs across the required layers and at least one experimental layer) plus every unit test under `tests/` and every benchmark assertion under `benchmarks/` of the reference implementation at the Standard's published commit. The reference suite is the primary conformance gate for second-party implementations; the unit-test and benchmark assertions are additional regression coverage on the reference implementation's behaviour.
...
- Expanded reference suite coverage of Layers 8, 9 and additional edge cases (minimum v1.0 coverage shipped in draft.10)
