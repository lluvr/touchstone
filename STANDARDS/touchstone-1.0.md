...
**Status:** Draft v0.10. All Section 4-12 content substantively complete, including Terminology (§2), Output structure (§4), Conformance declaration mechanism + invalidation criteria (§11), confirmed reference-test conformance bands (§8) disambiguated against the regression reference, structured Field positioning references (§12), falsifiable construct claims (§3.5) updated with three external corpus results × three task types × two head-to-head baselines (MiniCheck Flan-T5-Large and AlignScore-base) and 95% percentile bootstrap CIs on every Touchstone-side and AlignScore-side AUC, and Appendix C citing public-surface validation artifacts only. Section 6 (Specification Compliance) and Appendices A and B remain reserved for Standard 1.1. Independent editor review is pending.
**Version:** 1.0.0-draft.10
**Date:** 2026-05-16 (drafting in progress)
**License:** CC-BY 4.0
...
- **The "scoring substrate is independent" claim (§3.1).** Falsified if any non-Layer-1a layer's output is shown to depend on the choice of model that generated the input text in a way the layer's documented construct does not predict. The reference implementation runs identically on text from any generator; a measurement that shifts systematically with generator identity (beyond what Layer 1b/1c/10 are designed to surface) indicates an undocumented dependence. Status as of `1.0.0-draft.10`: Layer 6 inverse vocabulary proximity (the strongest surviving Touchstone signal across the external corpora tested) achieves AUC and 95% bootstrap CI as follows: RAGTruth Summary 0.6723 [0.6296, 0.7116], RAGTruth QA 0.6984 [0.6579, 0.7361], RAGTruth Data2Txt 0.6397 [0.6001, 0.6757], SummEval 0.7530 [0.7145, 0.7951], HaluEval summarization 0.7593 [0.7285, 0.7879]. Five (corpus, task) cells, all CIs disjoint from the 0.5000 chance level. Layer 4 unsourced_rate spikes to AUC 0.7603 [0.6907, 0.8260] on RAGTruth QA where number-density gates it in (n=277/900); on summary outputs the signal is gated out for most examples. Two independently-trained LLM-based baselines (MiniCheck Flan-T5-Large, AlignScore-base) bracket Layer 6 on the three summarization corpora: AlignScore-base 0.7368 [0.7006, 0.7699] / 0.8091 [0.7714, 0.8455] / 0.6879 [0.6567, 0.7187]; MiniCheck Flan-T5-Large 0.7125 / 0.8978* / 0.6752 (training-test leakage on SummEval; CIs deferred per §Limitations). On RAGTruth Summary and SummEval, both baselines outperform Layer 6 by 4-12 AUC points (the expected ordering for naturalistic hallucination detection). On HaluEval summarization, both baselines underperform Layer 6 (CIs disjoint on AlignScore vs Layer 6); the HaluEval result is partly explained by Layer 6 directly measuring what HaluEval's adversarial construction produces (vocabulary outside the article distribution), and is documented in the benchmark README. The two-baseline confirmation makes the HaluEval inversion a baseline-class limitation on adversarial vocabulary-shift corpora rather than a MiniCheck-specific weakness. No systematic model-identity dependence is observed beyond what label-balance and corpus-construction variation would predict.
