...
**Status as of 2026-05-24:** Touchstone is shipped at Standard 1.0.0-draft.15. One PyPI distribution is live: [`touchstone-mcp`](https://pypi.org/project/touchstone-mcp/) v0.1.2, a self-contained package that bundles the `clarethium_touchstone` reference library (internal version 0.2.1) and the MCP server, with `fastmcp` as its only third-party dependency. The artifact does the ruthless self-assessment in README `§Empirical validation` and `Limitations`. This handoff doc tells a fresh agent picking up the project what is done, what is explicitly out of scope, and where to push if you continue.
...
- **Reference implementation** under `src/clarethium_touchstone/` (Apache 2.0), bundled in the `touchstone-mcp` package on PyPI (library internal version 0.2.1). 452 library pytest tests pass; coverage at 97% with a 95% CI gate; ruff + mypy strict + Python 3.10 / 3.11 / 3.12 CI matrix green.
- **Touchstone MCP server** at `src/touchstone_mcp.py`, shipped inside the single [`touchstone-mcp`](https://pypi.org/project/touchstone-mcp/) package alongside the bundled library (`fastmcp` is the only third-party dependency). Four MCP tools (`verify`, `measure`, `assess_derivation_regime`, `list_modes`) exposed via the `touchstone-mcp` console script for any Model Context Protocol host (Claude Desktop, Claude Code, Cursor, custom). Host-wiring docs at `docs/mcp.md`. Migration history is documented in `CHANGELOG.md`.
...
- Install with `pip install touchstone-mcp` (single self-contained package: bundled `clarethium_touchstone` library + MCP server, `fastmcp` as its only third-party dependency). Add `[dev]` for the lint/type/test tooling or `[external]` for the benchmark runners.
- Working-tree development: clone, `pip install -e ".[dev]"`, then `pytest -q`. The one test suite exercises both the library and the MCP server surface.
