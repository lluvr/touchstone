...
- **Prompt cueing.** The xAI Grok prompt (`JUDGE_SYSTEM_PROMPT_CUED` in `benchmarks/external/judge_xai_from_pairs.py`) explicitly enumerates the six failure modes the §4 wall claim names as the substrate's blind spots: "Treat polarity flips, attribute swaps, scope shifts, time-frame shifts, relation reversals, and imputed causes as unsupported even when the vocabulary overlaps with the source." Every category in the 16-case fixture appears in that enumeration. The 16/16 separation rate measures the cued judge's ceiling on a fixture whose categories are listed in the prompt; it is not a generic frontier-judge capability claim. An in-session three-arm stress test (Claude Opus 4.7 as judge, cued vs blind vs source-removed, scores at `/tmp/stress_test_3arm_2026-05-18.json` in the session that authored draft.14) showed both cued and blind prompts at 16/16 on this fixture; the at-ceiling result means the toy fixture cannot measure the cueing effect, not that it is small. The blind prompt is shipped as `JUDGE_SYSTEM_PROMPT_BLIND` / `--prompt-variant blind`; rerun on a naturalistic-corpus sample to measure the cueing delta.
- **World-knowledge contamination.** The 16 cases use real entities (Apple, Tim Cook, Luca Maestri, fiscal-quarter dates). Grok 4.20 has seen Apple's filings in pretraining; it can flag "CFO Tim Cook" from priors alone without reading the source. The 16/16 separation rate cannot distinguish "judge reads source carefully" from "judge knows the world facts being perturbed." The in-session stress test referenced above measured priors-only separation by showing the judge only the OUTPUT (no SOURCE): 13/16 cases still separate, with 5/16 strongly separable from priors alone (`role_title_swap`, `fabricated_affiliation`, `scoping_shift`, `subtle_entity_swap`, `relation_reversal`) and 1/16 actively anti-separating (`magnitude_shift`: the source uses "$143 million" which contradicts the pretraining-known $100B+ Apple revenue range, so the faithful output looks more hallucinated than the hallucinated one to a knowledgeable judge). A clean version would use entirely fictional entities OR ablate the source on every test case. This stress test was a one-judge-one-session sanity check; the source-ablated Grok run remains future work.
...
The Verifier API supports this architecture: `Verifier(use_minicheck=True).score(...)` combines the cheap substrate signal with the caller-supplied trained-discriminator score into a single calibrated probability + signal breakdown + span localization. §4.3 measures the substrate's value-add when a frontier LLM judge is stage 2, with both the cued and blind judge prompts: the substrate-plus-judge linear blend's AUC value-add over judge-alone is meaningful on HaluEval (+0.020), marginal on RAGTruth (+0.005), and zero on SummEval-with-blind-judge (the CV picked α=0 on every fold). Substrate weight tracks substrate-only AUC strength per corpus. The "substrate complements judge" story holds on adversarial-summarization corpora where the substrate is itself strong; on cleaner NLI-shaped corpora a blind frontier judge is close to ceiling for what a small lexical substrate can add. Production teams should measure their own corpus α before deciding whether to integrate the substrate alongside a frontier judge.
