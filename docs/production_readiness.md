...
**Subsample-selection caveat: first-N is deterministic, not random.** The n=400 subsample is the first 400 rows of each corpus file in source order (`sampling_strategy: first_n_in_original_order`). This is reproducibility-friendly (no seed) and preserves base rate within ±1.5 pp on these corpora (per §4.2 first paragraph), but if the source file's first 400 rows happen to differ systematically from rows 400-900 (e.g., easier/harder topics, longer/shorter outputs, different annotator behavior across the labeling order), the §4.2 numbers carry that bias. **§4.2.4 across-subsample variance addresses this directly** by re-tabulating the substrate / MiniCheck / AlignScore on K=10 different prefix offsets (rows 0-399, 100-499, …, 900-1299 wrapped); reported per-detector F1 std across those K=10 offsets is the empirical bound on first-N-prefix bias. The std values are documented in §4.2.4. Random-sampling with multi-seed would be the more conventional design; first-N was chosen for reproducibility-without-seed, and the §4.2.4 across-subsample-variance compensation makes the design defensible but worth stating explicitly here.

**Substrate calibration leakage on RAGTruth Summary (load-bearing caveat).** The substrate's calibration coefficients in `DEFAULT_CALIBRATION_2026_05_17` were fit on a 70/30 stratified split of the RAGTruth Summary published test set (n_train=629, n_test=271, seed=0) per `src/clarethium_touchstone/_calibration.py`. Our n=400 first-N subsample of the same 900-row file overlaps the substrate's training set on roughly 280 rows (70% of 400). The substrate's reported RAGTruth-Summary AUC of 0.659 is therefore partially in-sample. Honest out-of-sample substrate AUC on RAGTruth (eval half of §4.3.1's stratified split, n=200): **0.633**. The +0.026 inflation is consistent with what a 6-parameter logistic regression overfits at n=629 (small but real). No analogous inflation exists for SummEval / HaluEval / RAGTruth-QA / RAGTruth-Data2Text (the calibration is genuinely OOD on those corpora; the +0.034 to +0.091 deltas between full-subsample and eval-half substrate AUC on those corpora are sampling variance, not calibration leakage). When citing the substrate's RAGTruth-Summary number externally, cite 0.633 (out-of-sample) or both numbers together; the §4.3.1 blend analysis on RAGTruth is honest because it uses the eval-half substrate predictions.
...
- The two-stage Verifier (`Verifier(mode="substrate_plus_minicheck").score(text, source, minicheck_supported_prob=...)`) MUST calibrate MiniCheck (and probably AlignScore, given its high MCE) before composition on corpora where the raw BSS is negative. Out-of-the-box composition of raw probabilities is unsafe.
...
- **Structural conflict (Move 6 disclosure):** the annotator (Claude Opus 4.7) and one of the panel judges (Anthropic Claude Sonnet 4.6, both cued and blind variants) are from the same model family. This means the per-case Sonnet judge scores reported in the table above were not blind to (independent of) the categorical labels assigned by the annotator. The Sonnet rows and the categorization column share a model-family prior; treat any per-category catch-rate comparison that involves Sonnet 4.6 as confounded. The cleanest fix is an external (different-family) annotator. **Status: Move 3 (label-noise floor at §4.2.9) was completed but with a same-family annotator (Claude Opus 4.7 via Agent dispatch) — Move 3 thus does NOT close this Move 6 cross-family conflict; both inherit the same-family bias.** A cross-family closure (Grok / Gemini / human annotator) is deferred. In the interim, prefer cross-family judge comparisons (Grok vs Claude, Gemini vs Claude) to within-family ones when interpreting this table, and treat §4.2.9's label-noise-floor estimates as lower bounds for the Claude judge specifically (the floor binds Claude's reported AUCs tighter than it binds Grok / GPT / Gemini judges).
...
- **Gemini probe data point.** Google Gemini-3-flash-preview was added via a local OpenAI-compatible proxy on a single corpus / variant: RAGTruth Summary blind n=200 prefix (`benchmarks/external/ragtruth_summary/results/judge_gemini_3_flash_blind_n200_2026-05-19.json`). Headline: **AUC 0.877 [0.815, 0.931]** at base rate 0.255 on this n=200 subsample. For comparison on the same RAGTruth corpus: Grok blind n=400 0.872, Claude Sonnet blind 0.899, GPT-5-mini blind 0.761. Gemini-3-flash sits between Grok and Claude on this single data point; not enough to characterize cross-corpus, but consistent with frontier-vendor cluster. Sequential proxy run (~37 s/pair under throttling); full 6-cell coverage was deferred because proxy throughput would have taken 11+ hours and the headline cross-vendor picture is already established at 3 vendors.

### 4.2.9 Independent LLM re-labeling: agreement with published labels

Every detector AUC / F1 reported in §4.2 / §4.3 is measured against the *published* hallucinated/faithful label on each pair. If those published labels are themselves noisy, then no detector — not even a perfectly calibrated oracle — can exceed roughly (1 − error_rate) accuracy against those labels. The headline numbers in §4.2 are therefore bounded above by some label-noise-related ceiling.

**Framing note (this section is not a canonical label noise floor measurement).** The canonical measurement of label noise is multi-human inter-annotator agreement, ideally reported by the corpus's original paper (Niu et al. for RAGTruth, Fabbri et al. for SummEval, Li et al. for HaluEval — each reports IAA that I have NOT yet pulled into this doc). This section reports something narrower and weaker: a single LLM annotator (Claude Opus 4.7 via Agent dispatch) agreeing with the published consensus labels at observed raw rates. Call that observed rate `r`. Treat the κ values below as an *LLM-vs-corpus* agreement metric, not an inter-human-annotator κ, and treat the "implied ceiling AUC" derivations as suggestive bounds that mix (a) genuine label noise, (b) the LLM annotator's systematic bias, (c) interpretive ambiguity at the hallucination/faithfulness boundary. The published-corpus IAA values are the proper noise floor; this section is the cheap probe.
...
**Bottom line.** Cohen's κ on all three corpora is in the substantial range (0.60-0.62) — for LLM-vs-corpus agreement, not for inter-human agreement. The implied LLM-agreement-derived ceiling AUC ranges from 0.60 (HaluEval) to 0.84 (SummEval). The strongest cued-judge held-out F1s reported in §4.2.8 sit close to or against the conservative bound on RAGTruth and HaluEval and well below the SummEval bound, **which is consistent with — but does not prove — frontier judges being at the corpus's label noise floor.** A proper proof would require citing the corpus's published multi-human IAA (deferred to a follow-up). Adopters who care about audit-grade detector performance on their *own* corpus should run an equivalent 50-pair LLM-relabeling probe on their own labels before trusting any AUC delta below ~0.05; this section is the protocol, not the canonical floor.

### 4.2.10 FactScore-class claim-level decomposition baseline

§4.2's detector panel includes substrate (lexical), trained NLI (MiniCheck, AlignScore), and frontier LLM judges (Grok, Claude, GPT, Gemini). The published SOTA approach for faithfulness evaluation — atomic claim decomposition with per-claim verification, as introduced by FactScore (Min et al. 2023) and RefChecker (Hu et al. 2024) — was not in the panel. This sub-section adds a FactScore-class baseline so the panel covers the canonical reference approach. Reproduce via `python -m benchmarks.external.factscore_baseline <pairs.json> --model grok-4.20-0309-non-reasoning --max-claims-per-output 10 --checkpoint-every 10 --output <out.json>` (one corpus at a time). Snapshots: `benchmarks/external/<corpus>/results/factscore_grok_n400_2026-05-19.json`. Comparison snapshot: `benchmarks/external/factscore_comparison_2026-05-20.json` (regenerable via `python -m benchmarks.external.factscore_vs_others`).

**Method.** For each (source, output) pair: (1) one LLM call asks Grok to extract atomic factual claims from the output as a JSON list, capped at 10; (2) for each extracted claim, one LLM call asks Grok whether the source supports the claim (binary plus confidence); (3) FactScore = 1 − mean(supported_per_claim), with higher meaning more hallucinated. Same xAI Grok 4.20-0309-non-reasoning model used as the LLM judge throughout the §4.2 panel, so vendor-effect is controlled for. All three corpora measured at n=400 on the same indices used elsewhere; runtime ~3-12 s/pair, ~$0.01-0.02/pair.

**Per-corpus comparison (in-sample F1-optimal; AUC + 95% CI; per-example latency / cost):**

| Corpus | FactScore (Grok-backend) | Single-step Grok cued | Single-step Grok blind | Single-step Claude cued | FactScore cost/pair |
|---|---|---|---|---|---|
| RAGTruth Summary | **0.728 [0.671, 0.793]**, F1 0.539 | 0.854, F1 0.670 | 0.872, F1 0.698 | 0.899, F1 0.754 | **$0.024** (11.8 s) |
| SummEval | **0.856 [0.789, 0.916]**, F1 0.636 | 0.933, F1 0.702 | 0.946, F1 0.763 | 0.921, F1 0.685 | **$0.010** (5.7 s) |
| HaluEval Summarization | **0.742 [0.693, 0.790]**, F1 0.699 | 0.807, F1 0.766 | 0.816, F1 0.761 | 0.824, F1 0.777 | **$0.017** (7.7 s) |

**Headline finding: FactScore-class claim-level decomposition does NOT beat single-step frontier judges on any of these three corpora.** The decomposition baseline underperforms its own backend LLM (single-step Grok) by 0.06-0.13 AUC across the three corpora, at substantially higher cost (~$0.01-0.02/pair vs ~$0.0005-0.001/pair for single-step Grok) and substantially higher latency (~6-12 s/pair vs ~1-2 s/pair). This is contrary to the conventional wisdom that "atomic-claim decomposition is the SOTA approach for faithfulness" and is worth careful interpretation.

**What this measurement does and does not say:**

- **It does say** that, on three English news-summarization corpora at n=400 with Grok 4.20-non-reasoning as the LLM backend, the FactScore protocol (10-claim cap, two-step extract-then-verify, mean-pooling) gives lower AUC than calling the same LLM with a single-step "is this output supported by source" prompt. The cost-per-AUC trade-off is unambiguously worse.
- **It does NOT say** that claim-level decomposition is generally worse than single-step. The published FactScore work uses biographical-text fact-checking where each output produces many independent claims and the per-claim verification is more focused than the whole-output verification. Summarization outputs are not that distribution: the output is dense, claims overlap, and the source is short enough that whole-output verification is tractable for a frontier LLM.
- **It does NOT control** for the choice of decomposition LLM, the choice of verification LLM, the claim-extraction prompt, the per-claim verification prompt, the max-claims-per-output cap (10), or the score-aggregation rule (1 − mean). All of these are knobs that a sophisticated FactScore implementation could tune. Our implementation uses one knob setting; published FactScore work likely tunes more carefully.
- **It does NOT establish** that decomposition is unnecessary in production; it establishes that on the specific corpora / specific LLM / specific protocol settings tested, the cost/benefit trade-off favors single-step. Adopters with longer or more claim-rich outputs (multi-paragraph essays, biographical text, code with many factual statements) should re-measure on their own distribution before generalizing this finding.

**Caveats:**

- Grok 4.20-non-reasoning was the LLM backend for both extraction and verification. A multi-vendor FactScore variant (e.g., Claude for extraction + GPT for verification) was not measured; results may differ.
- The 10-claim cap was reached on some long RAGTruth outputs; for those pairs the FactScore is biased toward "fewer-claims-checked" representativeness. The mean cap-hit rate is documented in the per-corpus snapshots.
- The single-step Grok comparison in the table above uses the cued prompt; blind is comparable but slightly different (see §4.2.8 for full vendor matrix).
- Claim extraction is itself an LLM judgment; "atomic factual claim" is interpretive. Two independent extraction runs with different temperature seeds would produce slightly different claim sets. Single-judge claim extraction is a known fragility of FactScore-class methods.

The §4.2 panel now has eight detector classes (substrate, MiniCheck, AlignScore, Grok cued, Grok blind, Claude cued, Claude blind, GPT-4o cued, GPT-5-mini cued, GPT-5-mini blind, Gemini blind, FactScore-Grok) on three corpora. FactScore-Grok joins MiniCheck and AlignScore as a "mid-tier" detector class — better than substrate, worse than single-step frontier judges, at intermediate cost.
...
- The substrate scores used here are the calibrated `Verifier(mode="substrate_only").score()` output, which was trained on RAGTruth Summary 70/30. On SummEval and HaluEval the substrate is out-of-distribution; in-distribution recalibration could move the substrate AUC up and shift the optimal alpha upward. The reported blend gains under blind judge are a lower bound on what corpus-calibrated substrate weights would deliver. **For RAGTruth specifically: the substrate column above is in-sample (calibration trained on this corpus's published test set); the honest OOD eval-half value is +0.026 lower (see §4.2 leakage caveat). §4.3.1's holdout-discipline numbers on RAGTruth use the eval-half substrate and are the OOD-honest reference.**
...
- **HaluEval-construction caveat (Move 5):** HaluEval's 50% base rate is by construction (Li et al. 2023 paired each source with exactly one synthetic hallucinated output). Lexical detectors that score "this output overlaps less with source than the typical faithful pair" are biased upward on this construction — every source has one faithful and one hallucinated output, so a detector that ranks "lower-overlap-than-pair-median" as more-likely-hallucinated wins by construction. The substrate's strong HaluEval AUC (0.748 in-sample / 0.714 OOD eval half) should be read with this caveat: HaluEval is the only corpus in the panel where the lexical detector beats trained NLI on AUC (§4.2.7 p=0.026 vs AlignScore; p=0.048 vs MiniCheck), and the construction may explain the asymmetry. RAGTruth and SummEval base rates are not constructed and reflect natural hallucination distributions.
...
- **Per-cluster AUC CIs are wide where cluster size or class balance is small** (the fresh-eyes self-check that should accompany any cluster-AUC table). Concrete examples: the 0.85-cluster substrate AUC of 0.803 on HaluEval is computed on 4 negatives × 52 positives; the AUC CI at n_neg=4 is approximately [0.50, 0.95] under stratified bootstrap. The 0.65-cluster (n_pos=42, n_neg=14) CI is narrower but still ~±0.15. The 0.0-cluster on SummEval (n_pos=2, n_neg=270) reports substrate AUC 0.161 with CI roughly [0.05, 0.50] — the *inversion* (AUC < 0.5) is robust even at this n because 2 of 2 positives sit at low substrate score, but the point estimate carries enormous variance. The verdict labels (CONFIRMED / REFUTED / Marginal) are robust to these CIs because they rely on the *pattern* (6 of 7 HaluEval clusters > 0.55; SummEval dominant cluster inverted) rather than on any single peak. Cite the cluster-level numbers and the weighted-mean as directional, not point-precise; cite the per-corpus verdicts as the load-bearing claims.
...
1. **Stage 1: Touchstone substrate (Verifier substrate-only)** — runs in <100 ms per output. Catches: lexically-distinguishable hallucinations (new numbers, new entities, new years, vocabulary drift). Routes outputs into a review queue ordered by suspicion score. Cheap enough to run on every output.
2. **Stage 2: A trained discriminator (MiniCheck Flan-T5-Large, AlignScore-base) OR an LLM-based judge (xAI Grok 4.20, Anthropic Claude Sonnet 4.6, OpenAI GPT-4o, Google Gemini-3-flash)** on the top X% of stage-1 outputs OR on every output if compute budget allows. Catches: semantically-distinguishable hallucinations (direction reversal, attribute swap, scoping shift, relation reversal, imputed cause). Trained discriminators and LLM judges are distinct detector classes with different cost / accuracy / latency profiles; §4.2.5 has the per-call cost table.
...
The Verifier API supports this architecture in four discriminated modes selected by which Stage-2 detector probability the caller supplies to `score()`. Calling `Verifier().score(text, source)` gives substrate-only mode (Stage-1 alone). Passing `minicheck_supported_prob=` auto-selects `substrate_plus_minicheck`; passing both `minicheck_supported_prob=` and `alignscore_supported_prob=` auto-selects `substrate_plus_minicheck_alignscore`; passing `judge_hallucinated_prob=` auto-selects `substrate_plus_judge` (added 2026-05-19 alongside the §4.2.8 / §4.3.1 multi-vendor work; linear blend `judge_alpha * substrate_prob + (1 - judge_alpha) * judge_hallucinated_prob` with `judge_alpha` defaulting to ≈ 0.3, a deliberate substrate-light default below the §4.3.1 holdout-blend tune-AUC-picked mean of ≈ 0.375; the picked α is strongly corpus-dependent, so tune on held-out data). All four modes return the same `VerifierResult` shape (probability + signal breakdown + span localization).

§4.3 measures the substrate's value-add when a frontier LLM judge is stage 2 under CV; §4.3.1 re-measures under honest tune/eval holdout. The holdout result revises the §4.3 claim downward: of the six Grok (corpus × judge-variant) cells, only HaluEval-blind shows a positive eval gain (+0.019 F1, +0.023 AUC at α=0.7 picked on tune-AUC). On the other five cells the blend is at parity with judge-alone or worse. The "substrate complements judge" story holds only on adversarial-summarization corpora where the substrate is itself strong on AUC (§4.2.7's HaluEval result); on cleaner NLI-shaped corpora a blind frontier judge is at the ceiling for what a small lexical substrate can add.

**Label noise floor compounds with the architecture recommendation (§4.2.9).** Cohen's κ on RAGTruth and HaluEval is 0.62 / 0.60, implying conservative ceiling AUCs of 0.68 / 0.60. The strongest cued-judge held-out F1s on those corpora (Claude / GPT-4o, both around 0.72-0.74) sit against the conservative ceiling — frontier judges are at the *label* noise floor, not the detection ceiling. The §4.3.1 HaluEval-blind blend gain (+0.019 F1, +0.023 AUC) lives inside the κ-implied label-noise band. Production teams considering whether to add the substrate alongside a frontier judge should treat that blend gain as suggestive, not confirmed, and should expect their own corpus-specific α-on-AUC measurement to come back small unless their labels are notably higher-quality than RAGTruth / HaluEval. For most corpus distributions the honest answer is "judge-alone is the deployable system; revisit if you have multi-annotator labels and a corpus where the substrate's AUC exceeds the trained-NLI baselines."
...
- `python -m benchmarks.external.substrate_within_judge_clusters` — computes the §4.3.2 mechanism check: within each Grok-blind score cluster (bin width 0.05), reports substrate AUC on within-cluster pairs to test whether the substrate genuinely tie-breaks judge clusters. Reads existing substrate + judge snapshots; no new API calls.
- Move 3 / §4.2.9 label-noise-floor snapshot at `benchmarks/external/label_noise_floor_n50_per_corpus_2026-05-19.json` was produced by an Agent-dispatched independent re-label of 50 random pairs per corpus (seed=`random.Random(42).sample(range(400), 50)`); the annotator was Claude Opus 4.7 via the Agent tool (no Anthropic API spend; the annotator IS the dispatched Claude instance reading source/output and recording a binary verdict + reason per pair). Reproduce by reading the snapshot's `per_pair_verdicts` list; the snapshot is fully self-contained for downstream re-analysis.
